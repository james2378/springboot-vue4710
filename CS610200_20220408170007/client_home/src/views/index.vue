		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/laundry_service/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_laundry_service = !show_list_laundry_service">
		                        <div class="switch_box">
		                            <span v-if="show_list_laundry_service"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--洗衣服务推荐列表组件 -->
		                <list_laundry_service v-if="show_list_laundry_service" :list="list_laundry_service" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_laundry_service.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_laundry_service" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_laundry_service" :key="i" @click="to_details('laundry_service',o,'laundry_service_id')">
		                            <td v-for="(oj,n) in fields_laundry_service">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_laundry_service: true,
				list_laundry_service: [],
				vm_arr_laundry_service: [
					"laundry_service_id",
					"clothing_items_",
					"clothes_type_",
					"clothing_map",
		...
		methods: {
			...
			// 获取洗衣服务列表
			get_laundry_service() {
				let url = "~/api/laundry_service/get_list?";
				url = "~/api/laundry_service/get_list?orderby=hits desc";
				this.$get(url, {
					...
					if (json.result) {
						this.list_laundry_service = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_laundry_service() {
		        var list = this.list_laundry_service;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_laundry_service.map((o) => {
		                // 第二个中括号是对象的属性
