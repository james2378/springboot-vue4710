		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','user_account') || $check_field('add','user_account') || $check_field('set','user_account')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="用户账户" prop="user_account">
						<div v-if="user_group !== '管理员'">
							...
							<!--<el-input id="business_name" v-model="form['user_account']" placeholder="请输入用户账户"-->
							<!--v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','user_account')) || (!form['application_postman_id'] && $check_field('add','user_account'))" :disabled="disabledObj['user_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user_account')">{{form['user_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','user_account')) || (!form['application_postman_id'] && $check_field('add','user_account'))" id="user_account" v-model="form['user_account']" :disabled="disabledObj['user_account_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','full_name')) || (!form['application_postman_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','id_number') || $check_field('add','id_number') || $check_field('set','id_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="身份证号" prop="id_number">
					<el-input id="id_number" v-model="form['id_number']" placeholder="请输入身份证号"
							  v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','id_number')) || (!form['application_postman_id'] && $check_field('add','id_number'))" :disabled="disabledObj['id_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','contact_number') || $check_field('add','contact_number') || $check_field('set','contact_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="联系电话" prop="contact_number">
					<el-input id="contact_number" v-model="form['contact_number']" placeholder="请输入联系电话"
							  v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','contact_number')) || (!form['application_postman_id'] && $check_field('add','contact_number'))" :disabled="disabledObj['contact_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','audit_status') || $check_field('add','audit_status') || $check_field('set','audit_status')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="审核状态" prop="audit_status">
					<el-select id="audit_status" v-model="form['audit_status']"
						v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','audit_status')) || (!form['application_postman_id'] && $check_field('add','audit_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','review_reply') || $check_field('add','review_reply') || $check_field('set','review_reply')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="审核回复" prop="review_reply">
					<el-input id="review_reply" v-model="form['review_reply']" placeholder="请输入审核回复"
							  v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','review_reply')) || (!form['application_postman_id'] && $check_field('add','review_reply'))" :disabled="disabledObj['review_reply_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','review_reply')">{{form['review_reply']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','reasons_for_application') || $check_field('add','reasons_for_application') || $check_field('set','reasons_for_application')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="申请理由" prop="reasons_for_application">
					<el-input type="textarea" id="reasons_for_application" v-model="form['reasons_for_application']" placeholder="请输入申请理由"
						v-if="user_group === '管理员' || (form['application_postman_id'] && $check_field('set','reasons_for_application')) || (!form['application_postman_id'] && $check_field('add','reasons_for_application'))" :disabled="disabledObj['reasons_for_application_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','reasons_for_application')">{{form['reasons_for_application']}}</div>
				</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/application_postman/upload?",
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/application_postman/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/application_postman/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/application_postman/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/application_postman/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/application_postman/view','get');
