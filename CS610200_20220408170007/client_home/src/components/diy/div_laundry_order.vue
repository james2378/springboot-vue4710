<template>
	<div class="diy_details_box diy_div_laundry_order">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "laundry_order_id",
					source_id: this.obj["laundry_order_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/laundry_order/set?laundry_order_id=' + _this.obj["laundry_order_id"], {
								praise_len
							}, (res) => {
								if(res.result){
