<template>
	<el-main class="bg table_wrap">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap">
					<el-form-item label="派送状态">
						<el-select v-model="query.dispatch_status">
                            <el-option v-for="o in list_dispatch_status" :key="o" :label="o"
                            	:value="o">
                            </el-option>
						</el-select>
					</el-form-item>
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_wrap_1">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/delivery_record/table','add') || $check_action('/delivery_record/view','add')" class="el-button el-button--default el-button--primary search_btn_add" to="./view?">添加
						</router-link>
            <el-button v-if="user_group == '管理员' || $check_action('/delivery_record/table','del') || $check_action('/delivery_record/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="delivery_fee" label="投递费用"
				v-if="user_group == '管理员' || $check_field('get','delivery_fee')" min-width="200">
			</el-table-column>
			<el-table-column prop="total_cost" label="合计费用"
			...
			<el-table-column prop="delivery_clerk" label="投递员"
				v-if="user_group == '管理员' || $check_field('get','delivery_clerk')" min-width="200">
				<template slot-scope="scope">
					{{ get_user_delivery_clerk(scope.row['delivery_clerk']) }}
				</template>
			</el-table-column>
			<el-table-column prop="dispatch_status" label="派送状态"
				v-if="user_group == '管理员' || $check_field('get','dispatch_status')" min-width="200">
			</el-table-column>
			<el-table-column prop="delivery_time" label="投递时间"
				v-if="user_group == '管理员' || $check_field('get','delivery_time')" min-width="200">
                <template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/delivery_record/table','set') || $check_action('/delivery_record/view','set') || $check_action('/delivery_record/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/delivery_record/table','set') || $check_action('/delivery_record/view','set') || $check_action('/delivery_record/view','get')"
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/delivery_record/get_list?like=0",
				url_del: "~/api/delivery_record/del?",
