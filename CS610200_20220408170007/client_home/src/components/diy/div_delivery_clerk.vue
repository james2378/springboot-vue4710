<template>
	<div class="diy_details_box diy_div_delivery_clerk">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "delivery_clerk_id",
					source_id: this.obj["delivery_clerk_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/delivery_clerk/set?delivery_clerk_id=' + _this.obj["delivery_clerk_id"], {
								praise_len
							}, (res) => {
								if(res.result){
