<template>
	<div class="diy_details_box diy_div_application_postman">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "application_postman_id",
					source_id: this.obj["application_postman_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/application_postman/set?application_postman_id=' + _this.obj["application_postman_id"], {
								praise_len
							}, (res) => {
								if(res.result){
