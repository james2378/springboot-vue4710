<template>
	<div class="diy_details_box diy_div_laundry_service">
		<div class="warp">
			<div class="container">
					...
					<div class="view praise_wrap">
						...
						{{ praiseLen }}
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/laundry_order/edit','add')" @click="to_form('/laundry_order/edit')" >预定服务 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/laundry_service/details','set')" @click="change_praise()" >点赞</button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/laundry_service/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "laundry_service_id",
					source_id: this.obj["laundry_service_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/laundry_service/set?laundry_service_id=' + _this.obj["laundry_service_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.clothing_items_
					query.img = this.obj.clothing_map
