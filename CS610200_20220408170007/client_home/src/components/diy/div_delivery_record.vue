<template>
	<div class="diy_details_box diy_div_delivery_record">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "delivery_record_id",
					source_id: this.obj["delivery_record_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/delivery_record/set?delivery_record_id=' + _this.obj["delivery_record_id"], {
								praise_len
							}, (res) => {
								if(res.result){
